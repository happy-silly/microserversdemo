        ...
        private void ConfigureConventionalControllers()
        {
            Configure<AbpAspNetCoreMvcOptions>(options =>
            {
                options.ConventionalControllers.Create(
                    typeof(BusinessApplicationModule).Assembly,
                    opt=>opt.RootPath="business");//配置Route path
            });
        }
        ...
        private static void ConfigureSwaggerServices(ServiceConfigurationContext context, IConfiguration configuration)
        {
            if (configuration["UseSwagger"] == "true")
            {
                context.Services.AddSwaggerGen(options =>
                {
                    ...
                    var xmlPath = Path.Combine(basePath, "Business.Application.xml");   //  添加 swagger xml 注释  这个xml文件开始是不存在的写上项目名.xml即可
